...
export function Transfer() {
    ...
    try {
      ...
      if('BarcodeDetector' in window) {
        ...
        reader.decodeFromVideoDevice(undefined, videoRef.current, async (result) => {
          ...
          const added=addTransferFrame(frame);
          setProgress({session:added.session,received:added.received,total:added.total,name:added.name});
          if(added.complete){ try { const rebuilt=await reconstructTransfer(added.session); if(rebuilt){setResult({url:rebuilt.url,name:rebuilt.name,size:rebuilt.size});stopReceive();} } catch(e){setError(e instanceof Error?e.message:'Transfer verification failed.');} }
        });
      }
  ...
  return <section className="mx-auto max-w-6xl py-8 sm:py-12">
    ...
    {tab==='send'?<div className="mt-5 grid gap-5 lg:grid-cols-[.85fr_1fr]">
      ...
      <div className="glass-panel rounded-[28px] p-5">{qr?<><div className="rounded-[28px] bg-white p-5"><img src={qr} alt="OR Transfer frame" className="mx-auto w-full max-w-[520px]"/></div><div className="mt-4 flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-bold">Frame {index+1} / {frames.length}</p><div className="flex flex-wrap gap-2"><button disabled={!frames.length} onClick={()=>setIndex(i=>Math.max(0,i-1))} className="rounded-full bg-white/10 px-4 py-2 text-sm">Prev</button><button disabled={!frames.length} onClick={()=>setIndex(i=>Math.min(frames.length-1,i+1))} className="rounded-full bg-white px-4 py-2 text-sm font-bold text-slate-950">Next</button><button disabled={frames.length<2} onClick={()=>playing?stopPlayback():setPlaying(true)} className="rounded-full bg-cyan-300 px-4 py-2 text-sm font-bold text-slate-950">{playing?'Pause':'Auto-play'}</button></div></div><div className="mt-3 flex items-center gap-3 text-xs text-[var(--text-muted)]"><span>Frame interval</span><select value={intervalMs} onChange={e=>setIntervalMs(Number(e.target.value))} className="rounded-full border border-white/10 bg-black/10 px-3 py-1.5"><option value="350">Fast · 350ms</option><option value="500">500ms</option><option value="700">Balanced · 700ms</option><option value="1000">1 sec</option><option value="1500">1.5 sec</option></select></div><p className="mt-3 text-center text-xs leading-5 text-[var(--text-muted)]">Display this QR on the sender and scan it with the receiving phone.</p></>:<div className="grid min-h-[520px] place-items-center text-center text-[var(--text-muted)]"><Radio size={36} className="mx-auto text-cyan-300"/><p className="mt-3 font-bold text-[var(--text)]">Transfer QR will appear here</p><p className="mt-1 text-sm">Choose a file to begin.</p></div>}</div>
